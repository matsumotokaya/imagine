...
> **Status: Phase 0/1/2 完了（2026-06-26）＋ R2 delete 経路を実装・本番デプロイ（2026-06-27, `r2-presign` v2 ACTIVE）。** production出力をR2化し Supabase を 1.8GB→約906MB に。新規publishも自動でR2。
> 本ドキュメントは履歴・教訓・**残フェーズの前提と着手チェックリスト**の記録として保持（削除しない）。
> 残フェーズ（default-images / banner資産 / library / 既存banner・upload）の前提だった **R2 delete経路は実装・本番デプロイ済み**（`supabase/functions/r2-presign` の `op:'delete'`、PUTと対称の権限チェック / クライアントは `src/utils/r2Upload.ts` の `deleteFromR2` / `deleteFileFromR2`）。残る前提は **provider列のDDL** と **バックフィル実行（R2 creds 投入）**。
> 関連: Gallery側のURL生成は単一リゾルバ `whatif-ep-xyz/src/lib/asset-url.ts`（`resolveAssetUrl`）に一元化済み（旧 `docs/IMAGE_URL_REFACTOR.md` は完了につき削除）。
...
## 0.2 Phase 2 完了（2026-06-26）— 既存 production のバックフィル＋削除

`scripts/backfill-production-to-r2.mjs`（冪等・dry-run既定）で実施。env は `.env.r2backfill.local`（gitignore）。
- **Pass A**: 既存 production 413ファイルを Supabase→R2 へコピー＋`production_outputs.storage_provider='r2'`（591行）。
- **Pass B**: Gallery `work_variants` の Supabase user-images feed URL を 53件 R2 URL へ書換（supabase残り0）。
- **削除**（`--delete-migrated --apply`）: R2存在を認証HEADで確認後、Supabase の production 元ファイル413件を削除。
- **結果**: Supabase user-images **1425MB→508.7MB**、合計 **約897MB（1GB未満達成）**。
- **重要な実装上の罠**: 検証HEADを公開CDNドメインに投げると Cloudflare のネガティブキャッシュで PUT 直後に偽の404 → **存在確認/検証は認証付き S3 API エンドポイントへのHEAD**（CDNキャッシュなし・read-after-write 強整合）で行うこと。

残: default-images(388MB) / 残り user-images(banner downloads・thumbnails・root uploads ~518MB) は後続フェーズ。**実測（2026-06-27, MCP read-only）: user-images 518MB / default-images 388MB ＝ 計約906MB が Supabase 残存。** R2 delete経路は実装・デプロイ済みになったため、残る前提は provider列DDL と バックフィル実行（R2 creds 投入）。

**Wave A = default-images 完了（2026-06-27）**: `default_images.storage_provider` 列を追加（DDL手動）、読み取りを `resolveAssetUrl` 経由に、新規アップロードをR2化（IMAGINE `85ee5f1`）。`scripts/backfill-default-images-to-r2.mjs` で原本+サムネ224件をR2コピー＋`provider='r2'`反転＋Supabase原本削除。**default-images 388MB→44MB / Supabase合計 906MB→約562MB**。残44MB(142obj)は参照外の孤立（別途 storage-cleanup）。**次wave = user-images(518MB)**: banner downloads(`banners.fullres_url`列)・thumbnails列・root upload(`elements` JSONB=最難所)を同パターンで。
...
**後続フェーズ**: R2 delete 経路（presigned DELETE）→ **実装・本番デプロイ済み（2026-06-27, `r2-presign` v2）**／banner assets の R2化／library(default_images・user_images) の provider列DDL＋読み取り更新／既存ファイルのバックフィル（Phase 2-4）。
